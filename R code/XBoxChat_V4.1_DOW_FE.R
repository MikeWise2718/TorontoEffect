...
mydata <- read.csv("../../data/chat_volume_duration_features-Aug2016-current.csv")
colnames(mydata)
...
newdata <- subset(newdata,IsSXC == "True")
...
newdata <- subset(newdata,newdata$dt >= "2016-09-10 00:00:00")
...
plot(daily$date_formatted, daily$ChatsInBlock,  main = "Xbox A&B Chat from VA: Raw Data at Daily Level", xlab = "Date", ylab = "Chat Initiations", pch=19)
...
newdata1$date_formatted<- as.Date(newdata1$dt, tz = "America/Los_Angeles")
m1 <- merge(daily,newdata1, by="date_formatted")
m1 <- m1[with(m1, order(dt)), ]
utils::View(m1)
m1$ChatsInBlock<-m1$ChatsInBlock.y
...
temp_data <- m1[c("dt","ChatsInBlock")]
