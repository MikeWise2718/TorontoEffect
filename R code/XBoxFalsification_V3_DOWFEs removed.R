...
Xdata <- subset(mydata,Subject == "TS - Xbox Games & Apps")
...
daily <- subset(daily,daily$date_formatted <= "2017-01-07 00:00:00")
...
newdata1$date_only<- as.Date(newdata1$dt)
...
##################################################
###############################
# Choose one week on either side of these breaks  First remove all DOW FEs since do important.
#
# Third break
###############################
#m2<-subset(m1,m1$dt < "2016-12-23 00:00:00")

utils::View(m2)
m2 <-subset(m1,m1$dt < "2016-12-23 00:00:00")

#m2<-m1
reg_hourly_resid_poly8 = glm(ChatsInBlock ~ as.factor(ChatDOW), data=m2)
m2$ChatsInBlock_DOW <- resid(reg_hourly_resid_poly8)
#m2<-m1
...
temp_data <- msky3[c("dt","ChatsInBlock_DOW")]
daily <- aggregate(. ~ cut(temp_data$dt, "1 day"), temp_data[setdiff(names(temp_data), "dt")], sum)
colnames(daily)[1] <- "daily_time"
...
#daily$date_formatted <- as.Date(daily$daily_time)
daily$linear1 <- seq(length(daily$ChatsInBlock_DOW))
...
daily <-subset(daily,daily$daily_time < "2016-11-08 00:00:00")
msky3 <-subset(msky3,msky3$dt < "2016-11-08 00:00:00")
...
msky3_linear <-subset(msky3_linear,msky3_linear$dt < "2016-11-06 00:00:00")
...
##### Linear regression
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour)  + linear1+ linear2+ sky30_50 , data=msky3_linear)
...
reg_linear_sky3 = glm(ChatsInBlock_DOW  ~ as.factor(Hour)  + linear1+ linear2, data=msky3_linear)
...
#Create linear time trend before and after
temp_data <- msky2[c("dt","ChatsInBlock_DOW")]
daily <- aggregate(. ~ cut(temp_data$dt, "1 day"), temp_data[setdiff(names(temp_data), "dt")], sum)
colnames(daily)[1] <- "daily_time"
...
##### Linear regression
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour)  + linear1+ linear2+ sky10_30, data=msky2_linear)
summary(reg_linear_sky3)
coeftest(reg_linear_sky3, vcov = vcovHC(reg_linear_sky3, "HC1"))
...
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour)  + linear1+ linear2, data=msky2_linear)
...
##### Linear regression
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour)  + linear1+ linear2+ sky50_90, data=msky2_linear)
summary(reg_linear_sky3)
coeftest(reg_linear_sky3, vcov = vcovHC(reg_linear_sky3, "HC1"))
...
#Raw Data
plot(msky2_linear$dt, msky2_linear$ChatsInBlock_DOW,  main = "Chat Initiations", xlab = "Date", ylab = "Chats/Hour")
abline(v=as.numeric(msky2_linear$dt[sky2_only]),col = "red")
...
##### Linear regression
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour) + ChatDOW + linear1+ linear2+ sky50_90, data=msky2_linear)
summary(reg_linear_sky3)
coeftest(reg_linear_sky3, vcov = vcovHC(reg_linear_sky3, "HC1"))
...
##### Linear RESIDUAL PLOT on SKy2
sky2_only<-which(grepl("2016-12-15 00:00:00", msky2_linear$dt))
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour)  +ChatDOW + linear1+ linear2, data=msky2_linear)
msky2_linear$resid_linear_sky3 <- resid(reg_linear_sky3)
plot(msky2_linear$dt, msky2_linear$resid_linear_sky3,  main = "Chat Initiations", xlab = "Date", ylab = "Residual Live Chats/Hour (Unique Linear trends, no dummy)")
abline(v=as.numeric(msky2_linear$dt[sky2_only]),col = "red")
...
##### Linear regression
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour) +  linear1+ linear2+ content, data=msky2_linear)
summary(reg_linear_sky3)
coeftest(reg_linear_sky3, vcov = vcovHC(reg_linear_sky3, "HC1"))
...
reg_linear_sky3 = glm(ChatsInBlock_DOW ~ as.factor(Hour)   + linear1+ linear2, data=msky2_linear)
msky2_linear$resid_linear_sky3 <- resid(reg_linear_sky3)
plot(msky2_linear$dt, msky2_linear$resid_linear_sky3,  main = "Chat Initiations", xlab = "Date", ylab = "Residual Live Chats/Hour (Unique Linear trends, DOW FEs Removed)")
abline(v=as.numeric(msky2_linear$dt[sky2_only]),col = "red")
...
#Raw Data
plot(daily$daily_time, daily$ChatsInBlock_DOW,  main = "Chat Initiations DOW FEs removed", xlab = "Date", ylab = "Chats/Hour")
