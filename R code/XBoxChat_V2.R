...
setwd("C:/Users/jlariv/OneDrive - Microsoft/Toronto/Data/Old")
...
newdata3 <- newdata2
ag1 <- aggregate(. ~ cut(newdata3$dt, "1 hour"), newdata4[setdiff(names(newdata3), "dt")], sum)
ag2 <- aggregate(. ~ cut(newdata3$dt, "1 hour"), newdata4[setdiff(names(newdata3), "dt")], mean)
...
m1$count11 <- m1$count^11
m1$count12 <- m1$count^12
