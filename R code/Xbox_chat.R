...
#newdata3 <- subset(newdata2,newdata2$dt <= "2016-11-14 00:00:00")
...
m1$count8 <- m1$count^8
m1$count9 <- m1$count^9
m1$count10 <- m1$count^10
...
reg_poly9_full = glm(ChatsInBlock ~ as.factor(Hour) + as.factor(ChatDOW) + count + count2 + count3 + count4 + count5 + count6 + count7 + count8 + count9 + dum1 + dum2 + dum3 + dum4, data=m1)
summary(reg_poly9_full)
coeftest(reg_poly9_full, vcov = vcovHC(reg_poly9_full, "HC1"))
...
#10th order polynomial Residual
reg_poly10_full_resid = glm(ChatsInBlock ~ as.factor(Hour) + as.factor(ChatDOW) + count + count2 + count3 + count4 + count5 + count6 + count7 + count8 + count9 + count10, data=m1)
m1$full_resid_poly10 <- resid(reg_poly10_full_resid)
coeftest(reg_poly10_full, vcov = vcovHC(reg_poly10_full, "HC1"))
