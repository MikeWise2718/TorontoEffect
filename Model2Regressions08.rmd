---
title: "Model-DID Regressions - V03"
author: "Mike Wise"
date: "29 April 2017"
...
getregdfdid <- function(odf,vname,vname1,vname2,dates){
  ...
  df <- odf[c("dt","hour","dow",vname1,vname2)]
  df$HOD <- df$hour
  df$DOW <- df$dow
  df$dow <- NULL
  df$hour <- NULL
...
regressoverdates <- function(model,df,vname,area,title,befdays=7,aftdays=7){
  ...
  for (chgdate in dates){
    switch(model,
    "model1"= regress1(condf,vname,area,backg,dates,chgdate,atit,befdays=7,aftdays=7),
    "model2"=regress2(condf,vname,area,backg,dates,chgdate,atit,befdays=28,aftdays=28),
    "model-did"=regress2did(condf,vname,area,backg,dates,chgdate,atit,befdays=28,aftdays=28)
    )
  }
}
```
...
# we just want call rates for now - 25.04.2017
#  regressoverdates(m,condf,"winchib","SMC","Hourly Chats")
