---
title: "Read Chat Call Sessions and Merge"
author: "Mike Wise"
date: "16 Mar 2017"
...
readCallVolume <- function(frdate,todate,fname){
  ...
  lcdf  <- lcdf %>% mutate( dt = as.POSIXct(lcdf$DateHour,format="%m/%d/%Y %I:%M:%S %p",tz=tztz)) %>%
                    mutate( dnum = round(as.numeric(difftime(dtd,firstday,"days")))) %>%
                    mutate( hnum = dnum*24 + hour(dt)) %>%
                    arrange( dt ) # sort
  ...
  lcdf <- lcdf %>% group_by(hnum) %>% 
                   summarise(dt=min(dt),dnum=min(dnum),
                             wincall=sum(WindowsCalls),xbxcall=sum(XboxTotalCalls+XboxABCalls)) %>%
...
dailyplot(ecdf,"dt","calls",series="series",mtit="Total Call Volumne - Windows vs. XBOX vs. XBOX AB",ylab="Call Count",vlines=totdates,backg=totback)
```
...
```{r echo=F}
condf <- tdf[,c("dt","dnum","hnum","totchib","winchib","xbxchib","totcall","wincall","xbxcall","actsess","actuser")]
