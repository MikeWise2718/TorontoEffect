---
title: "Experiements Early 2017 - Read Chat Call Sessions and Merge 0.3"
author: "Mike Wise"
date: "27 April 2017"
...
smcb2dates <- c("2017-04-17 06:00/purple/100%/2017-04-17 08:00",
                ...
                "2017-04-22 08:00/purple/100%**/2017-04-22 09:00",
                "2017-04-22 18:00/purple/100%**/2017-04-22 19:00",
                "2017-04-23 06:00/purple/100%/2017-04-23 07:00",
                "2017-04-23 19:00/purple/100%/2017-04-23 20:00")
...
crackdate <- function(datestr){
  ...
  return(list(sdate=date,ssdate=ssdate,edate=date,sedate=sedate,val=val,sval=sval,pctpresent=pctpresent))
}
...
readSessionVolume <- function(frdate,todate,fname){
  ...
  print(sprintf("Dates min:%s max:%s",min(svdf$dt),max(svdf$dt)))
...
tdf <- full_join(cccc,svdf,by="hnum",suffix=c(".cccc",".sess"))
print(sprintf("cccc-sess join merged %d x %d rows to %d rows",nrow(cccc),nrow(svdf),nrow(tdf)))

# fixup missing session dates
tdf[is.na(tdf$dt),]$dt <- tdf$dt.chat[is.na(tdf$dt)]
tdf[is.na(tdf$dnum),]$dnum <- tdf$dnum.chat[is.na(tdf$dnum)]
