...
version <- 5.1
versionstring <- sprintf("Version %.1fa",version)
...
newdata$dt <- as.POSIXct(newdata$dtt, format = "%m/%d/%Y %H:%M",tz=tztz)
newdata <-newdata[with(newdata, order(newdata$dt)), ]
newdata <- subset(newdata,newdata$dt >= as.POSIXct("2016-07-15 00:00:00",tz=tztz))
newdata <- subset(newdata,newdata$dt < as.POSIXct("2017-01-08 00:00:00",tz=tztz))
...
newdata$dt <- as.POSIXct(newdata$dtt, format = "%m/%d/%Y %H:%M",tz=tztz)
newdata <-newdata[with(newdata, order(newdata$dt)), ]
newdata <- subset(newdata,newdata$dt >= as.POSIXct("2016-07-27 00:00:00",tz=tztz))
newdata <- subset(newdata,newdata$dt < as.POSIXct("2017-01-08 00:00:00",tz=tztz))
```
...
newdata1 <- subset(newdata,newdata$dt >= as.POSIXct("2016-10-01 00:00:00",tz=tztz))
...
m1$sky0_10 <- ifelse(m1$dt >= as.POSIXct("2016-10-11 00:00:00",tz=tztz),1,0)
m1$sky10_30 <- ifelse(m1$dt >= as.POSIXct("2016-10-18 00:00:00",tz=tztz),1,0)
m1$sky30_50 <- ifelse(m1$dt >= as.POSIXct("2016-11-01 00:00:00",tz=tztz),1,0)
m1$content <- ifelse(m1$dt >= as.POSIXct("2016-11-17 00:00:00",tz=tztz),1,0)
m1$sky50_90 <- ifelse(m1$dt >= as.POSIXct("2016-12-15 00:00:00",tz=tztz),1,0)
#####
#Location in column where rollout occurs
...
```{r}
m2<-subset(m1,m1$dt < as.POSIXct("2016-12-24 00:00:00",tz=tztz))
...
msky3 <-subset(m2,m2$dt >= as.POSIXct("2016-10-25 00:00:00",tz=tztz))
msky3 <-subset(msky3,msky3$dt < as.POSIXct("2016-11-08 00:00:00",tz=tztz))
...
daily <-subset(daily,daily$daily_time < as.POSIXct("2016-11-08 00:00:00",tz=tztz))
msky3 <-subset(msky3,msky3$dt < as.POSIXct("2016-11-08 00:00:00",tz=tztz))
...
# set.seed(1234)
# nrow(msky3_linear)
# colnames(msky3_linear)
# sample_n(msky3_linear,20)
```
...
msky2 <-subset(m2,m2$dt >= as.POSIXct("2016-10-13 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-10-25 00:00:00",tz=tztz))
...
#Daylight savings time for this break makes things a bit odd to deal with so must drop extra dates
daily <-subset(daily,daily$daily_time < as.POSIXct("2016-10-23 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-10-23 00:00:00",tz=tztz))
...
msky2 <-subset(m2,m2$dt >= as.POSIXct("2016-12-10 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-12-21 00:00:00",tz=tztz))
msky2$sky10_30 <- ifelse(msky2$dt >= break2,1,0)
...
#Daylight savings time for this break makes things a bit odd to deal with so must drop extra dates
daily <-subset(daily,daily$daily_time < as.POSIXct("2016-12-20 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-12-20 00:00:00",tz=tztz))
...
break2<-"2016-12-15 00:00:00"
break2_date<-"2016-12-15"
msky2 <-subset(m2,m2$dt >= as.POSIXct("2016-12-08 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-12-23 00:00:00",tz=tztz))
msky2$sky10_30 <- ifelse(msky2$dt >= break2,1,0)
...
#Daylight savings time for this break makes things a bit odd to deal with so must drop extra dates
daily <-subset(daily,daily$daily_time < as.POSIXct("2016-12-23 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-12-23 00:00:00",tz=tztz))
...
msky2 <-subset(m2,m2$dt >= as.POSIXct("2016-11-13 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-11-22 00:00:00",tz=tztz))
msky2$sky10_30 <- ifelse(msky2$dt >= break2,1,0)
...
#Daylight savings time for this break makes things a bit odd to deal with so must drop extra dates
daily <-subset(daily,daily$daily_time < as.POSIXct("2016-11-21 00:00:00",tz=tztz))
msky2 <-subset(msky2,msky2$dt < as.POSIXct("2016-11-21 00:00:00",tz=tztz))
