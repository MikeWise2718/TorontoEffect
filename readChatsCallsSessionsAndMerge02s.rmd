---
title: "Read Chat Call Sessions and Merge 0.2"
author: "Mike Wise"
date: "26 April 2017"
...
readChatVolume <- function(frdate,todate,fname){
 ...
 # lcdf$temp_date<- as.POSIXct(lcdf$Date,format="%m/%d/%Y %I:%M:%S %p",tz=tztz)
  ...
  lcdf <- lcdf[,c("dt","dnum","hnum","IsSMC","ChatsInBlock")]
...
ecdf <- cldf %>% group_by(dnum) %>% 
                 summarise(dt=min(dt),xbox_total=sum(xtkcall+xabcall),xbox_tech=sum(xtkcall),xbox_ab=sum(xabcall),xbox_diff=sum(xabcall)-sum(xtkcall)) %>%
                 mutate( dnum=NULL ) %>% # delete dnum col
                 gather(series,calls,-dt)

clrs <- c("xbox_total"="blue","xbox_tech"="darkgreen","xbox_ab"="purple","xbox_diff"="red")

dailyplot(ecdf,"dt","calls",series="series",mtit="Total Call Volumne - Xbox Tech vs. XBOX AB with Sum vs. Diff",ylab="Call Count",vlines=xtotdates,backg=xbxback,clrs=clrs)
```


## Xbox diff call volume plots

```{r fig.width=14, fig.height=6, echo=T}
ecdf <- ecdf %>% filter(series=="xbox_diff")
dailyplot(ecdf,"dt","calls",series="series",mtit="Total Call Volumne - Xbox Tech diff XBOX AB",ylab="Call Count",vlines=xtotdates,backg=xbxback,clrs=clrs)
```
